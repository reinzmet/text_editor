...
typedef struct erow { 		//erow is editor rows and will store row of text in our editor
	int size;
	char *chars;
} erow;
...
int editorReadKey() {
	...
	if (c == '\x1b') {			//if the character is an arrow key, we must handle the escape sequence first
		char seq[3];
...
int getWindowSize(int *rows, int *cols) {
	...
	if (ioctl(STDOUT_FILENO, TIOCGWINSZ, &ws) == -1 || ws.ws_col == 0) { 		//TIOCGWINSZ stands for terminal input/output control get windwow sizek
...
void abAppend (struct abuf *ab, const char *s, int len) { 		// appends the strings s to abuf after dynamically allocating memory using realloc 
...
void abFree(struct abuf *ab) { 				// destructor which deallocates the dynamic memory used by abuf
	free(ab->b);
}
		...
		if (y >= E.numrows) {
			if (E.numrows == 0 && y == E.screenrows / 3)
			{
				...
				int welcomelen = snprintf(welcome, sizeof(welcome),
										  "SUper editor -- version %s", KILO_VERSION);
...
void editorRefreshScreen() {				//inits the buffer, so we dont need to write for every screen refresh
